...
import VeoAnimationView from '../admin/VeoAnimationView';
import { DashboardIcon, PosIcon, TableIcon, OrdersIcon, KdsIcon, ProductsIcon, ReportsIcon, UsersIcon, MovieIcon, MenuIcon, TicketIcon } from '../icons';
...
const viewLabels: Record<AdminView, string> = {
  ...
  [AdminView.VeoAnimation]: 'Animação (Veo)',
};
...
const AdminScreen: React.FC<AdminScreenProps> = ({ menuItems, setMenuItems, orders, setOrders, tables, setTables, customers, setCustomers, coupons, setCoupons }) => {
  ...
  const renderView = () => {
    switch (activeView) {
      ...
      case AdminView.VeoAnimation:
        return <VeoAnimationView />;
      default:
        return <Dashboard orders={orders} />;
    }
  };
      ...
      <aside className={`fixed inset-y-0 left-0 z-30 w-64 bg-brand-dark text-white flex flex-col p-4 transform transition-transform duration-300 ease-in-out lg:static lg:translate-x-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <nav className="flex-1 overflow-y-auto">
          <ul className="space-y-2">
            ...
            <NavItem view={AdminView.VeoAnimation} icon={<MovieIcon className="h-6 w-6"/>} label="Animação (Veo)" />
          </ul>
        </nav>
      </aside>
